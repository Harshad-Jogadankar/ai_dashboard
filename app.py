...
def get_ai_insights(df):
    """
    Generates insights about the data using Azure OpenAI (e.g., GPT-4o).
    
    Args:
        df (pd.DataFrame): The DataFrame to analyze.
        api_key (str): Your Azure OpenAI API key.
        api_base (str): Your Azure OpenAI API base URL (e.g., "https://your-resource-name.openai.azure.com/").
    
    Returns:
        str: A string containing the AI-generated insights.
    """
    ...
    client = AzureOpenAI(  
        ...
        azure_endpoint="https://fy26-hackon-q1.openai.azure.com/openai/deployments/RazorHustlers/chat/completions?api-version=2025-01-01-preview"
    )
...
def get_dataframe_hash(df):
    """Returns a unique hash for the dataframe content."""
...
def main():
    ...
    # Initialize session state cache if not already
    if "insights_cache" not in st.session_state:
        st.session_state["insights_cache"] = {}

    # Only generate insights if not already cached
    if df_hash not in st.session_state["insights_cache"]:
        with st.spinner("Generating AI insights..."):
           insights = get_ai_insights(df, api_key, api_base)
            st.session_state["insights_cache"][df_hash] = insights
            
    # Always show the cached insights
    st.markdown(st.session_state["insights_cache"][df_hash])
